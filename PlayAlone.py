    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand)!=0: # As long as there are still letters left in the hand:
        ...
        if word == '.':# If the input is a single period:
            ...
            print("Goodbye! Total score: {0} points.".format(total_score))# End the game (break out of the loop)
            if i==0:
                return None
            break
            ...
            if not isValidWord(word,hand,wordList):# If the word is not valid:
                ...
                total_score+=score
                print('{0} earned {1} points. Total: {2} points'.format(word,score,total_score))# Tell the user how many points the word earned, and the updated total score, in one line followed by a blank line
                
                hand=updateHand(hand,word)# Update the hand
            i+=1

    print('Run out of letters. Total score: {0} points.'.format(total_score))# Game is over (user entered a '.' or ran out of letters), so tell user the total score
